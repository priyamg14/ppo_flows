...
@hydra.main(config_path="", config_name="config_beacon", version_base="1.1")
def main(cfg: "DictConfig"):  # noqa: F821
    ...
    from utils_beacon import eval_model, make_parallel_env, make_ppo_models
    ...
    collector = SyncDataCollector(
        create_env_fn=make_parallel_env(cfg.env.env_name, cfg.env.num_envs, cfg, "cpu"),
        policy=actor,
    ...
    test_env = make_parallel_env(cfg.env.env_name, 1, cfg, device, is_test=True)
    if logger_video:
        test_env = test_env.append_transform(
            VideoRecorder(logger, tag="rendering/test", in_keys=["observation"])
        )
    test_env.eval()
    ...
    for i, data in enumerate(collector):
        ...
        log_info = {}
        # print("data next reward:    ", data["next","reward"])
